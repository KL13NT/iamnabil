...
const Item = ({ item, id, expanded, setExpanded }) => {
	...
	return (
		<li class='list-none m-0 p-0 border-b-[1px] transition-colors transition-duration-200 border-ie-gray-800 hover:border-transparent'>
			<button
				{...getToggleProps({
					onClick: () => setExpanded(id)
				})}
				data-active={expanded === id}
				class='w-full text-3xl py-8 px-4 flex justify-between gap-2 flex-wrap text-right font-bold transition-colors transition-duration-200 hover:bg-[#ebebeb] data-[active=true]:bg-[#ebebeb] rounded'
			>
				<span class='md:max-w-[90%]'>{item.title}</span>
				<span class='text-2xl font-normal'>
					{formatDuration(item['itunes:duration'])}
				</span>
			</button>
			<div {...getCollapseProps()} class='px-4'>
				...
				{expanded === id && (
					<iframe
						...
						class='h-[100px] md:h-[161px] w-full mt-12 mb-8'
						height='161px'
						width='800px'
						border='0'
					></iframe>
